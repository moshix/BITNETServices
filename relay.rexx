...
/*  v3.1.0  :  Show last 20 chat lines upon login or /history cmd    */
...
history.0=20             /* history goes back 20 last chat lines */
      ...
      /*  USER IS ALREADY LOGGED ON */
             do ci=1 to words($.@)
               ...
               /* send msg to orignal sender in special format */
                 if userid \= "RSCS" & loopCondition =0 then ,
                      'TELL' cuser 'AT' cnode '-->'msg
              end
               else do
         /* originally we checked here for config parm:send2ALL=1   */
                if userid \= "RSCS" & loopCondition = 0 ,
                 then  'TELL' cuser 'AT' cnode '<> 'userid'@'node':'msg
                      history.historypointer = msg
                      if historypointer = history.0 then do
                         historypointer = 1
                      else do
...
do  i = 0 to history.0      by 1
if history.i = "" then do
   'TELL 'userid' AT 'node 'History.'i' : 'history.i
end
return 0
